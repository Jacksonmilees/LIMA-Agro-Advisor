...
import { Bell, ClipboardList, CloudRain, LayoutDashboard, LineChart, MessageCircle, Mic, ShieldCheck, Sprout, UserCog } from 'lucide-react';
...
const Navigation: React.FC<NavigationProps> = ({ currentView, onNavigate, userProfile, unreadNotifications, onOpenNotifications, language }) => {
  ...
  return (
    <>
      ...
      <nav id="main-navigation" className="md:hidden fixed bottom-6 left-4 right-4 bg-stone-900/90 dark:bg-stone-800/95 backdrop-blur-xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.3)] z-50 h-16 rounded-2xl flex items-center justify-between px-2 safe-area-bottom overflow-x-auto custom-scrollbar">
        {navItems.map((item) => {
      ...
      <nav id="main-navigation" className="hidden md:flex bg-emerald-950/90 dark:bg-stone-900/95 backdrop-blur-xl border-b border-white/10 text-white shadow-2xl z-50 sticky top-0 px-8 h-20 items-center justify-between transition-all">
        {/* Logo Area */}
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => onNavigate(item.id)}
                className={`group relative flex items-center space-x-2 px-3 py-2.5 rounded-2xl transition-all duration-300 text-sm font-medium overflow-hidden ${isActive
